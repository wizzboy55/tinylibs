...
#define TINYWATCHDOG_SETFREQUENCY(frequency)	ccp_write_io((void *)&(WDT.CTRLA), frequency | WDT_WINDOW_OFF_gc)

#define TINYWATCHDOG_WAITFORSYNC()			while(WDT.STATUS & WDT_SYNCBUSY_bm) {}

#define TINYWATCHDOG_ENABLE(frequency)	do {		\
	TINYWATCHDOG_WAITFORSYNC()						\
	TINYWATCHDOG_SETFREQUENCY(WDT_PERIOD_OFF_gc);	\
	TINYWATCHDOG_WAITFORSYNC()						\
} while(0)
